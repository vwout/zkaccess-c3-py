...
class ControlDeviceBase(ABC):
    ...
    def __repr__(self):
        return "\r\n".join(
            [
                "%-12s %-10s (%s)"
                % ("operation", self.operation, repr(self.operation)),
...
class ControlDeviceOutput(ControlDeviceBase):
    def __init__(
        self, output_number: int, address: consts.ControlOutputAddress, duration: int
    ):
        ControlDeviceBase.__init__(
            self, consts.ControlOperation.OUTPUT, output_number, address, duration
        )
        ...
        return "\r\n".join(
            [
                ...
                % (
                    "param2",
                    self.param2,
                    repr(consts.ControlOutputAddress(self.address)),
                ),
...
class ControlDeviceCancelAlarms(ControlDeviceBase):
    ...
    def __repr__(self):
        return "\r\n".join(
            ["ControlDevice Cancel Alarm Operation:", ControlDeviceBase.__repr__(self)]
        )
...
class ControlDeviceNormalOpenStateEnable(ControlDeviceBase):
    def __init__(self, door_number: int, enable: bool):
        ControlDeviceBase.__init__(
            self, consts.ControlOperation.ENDIS_NO_STATE, door_number, enable
        )
    ...
    def __repr__(self):
        return "\r\n".join(
            [
                ...
                "%-12s %-10s %s"
                % ("param2", self.param2, "Enable" if self.enabled else "Disable"),
            ]
        )
...
class ControlDeviceRestart(ControlDeviceBase):
    ...
    def __repr__(self):
        return "\r\n".join(
            ["ControlDevice Restart Operation:", ControlDeviceBase.__repr__(self)]
        )
